...
public class GeofencingCalculatedFieldState extends BaseCalculatedFieldState {
    ...
    public ListenableFuture<CalculatedFieldResult> performCalculation(EntityId entityId, CalculatedFieldCtx ctx) {
        ...
        OutputType type = ctx.getOutput().getType();
        var result = new CalculatedFieldResult(ctx.getOutput().getType(), ctx.getOutput().getScope(), toResultNode(type, valuesNode));
    ...
    private JsonNode toResultNode(OutputType type, ObjectNode valuesNode) {
        if (OutputType.ATTRIBUTES.equals(type) || latestTimestamp == -1) {
            return valuesNode;
        }
